...
export default function EmployeeProjectDetailPage() {
  ...
  // Form states for Credentials Edit
  const [devUrl, setDevUrl] = useState('');
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [notes, setNotes] = useState('');

  // Fetch Project Details (includes assignments & credentials if available)
  const { data: project, isLoading, error } = useQuery<ProjectDetail>({
    queryKey: ['employee-project', id],
    queryFn: async () => {
  ...
  const saveCredentialsMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['employee-project', id] });
      ...
      <PageWrapper>
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-zinc-505 dark:text-zinc-500 max-w-sm">
              {error instanceof Error ? error.message : 'You may not have permission to view this project.'}
            </p>
            <Button
              variant="outline"
  ...
  return (
    <PageWrapper>
      ...
      <div className="premium-card relative bg-card border border-border rounded-2xl overflow-hidden shadow-card p-6 mb-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span className="bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider">
            ...
            <p className="text-xs text-zinc-450 dark:text-zinc-500 mt-1">
              Client: <span className="font-semibold text-zinc-700 dark:text-zinc-350">{project.clientName}</span>
              {project.clientEmail && ` (${project.clientEmail})`}
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-4 bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-150 dark:border-zinc-800 rounded-xl p-4 shrink-0 self-start md:self-auto">
            {/* Progress Adjuster */}
            <div className="space-y-1.5 pr-2 border-r border-zinc-200 dark:border-zinc-850">
              <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">
                <span>Progress</span>
                <span className="text-emerald-600 dark:text-emerald-400 text-xs">{project.progress}%</span>
              </div>
              <input
                type="range"
                min="0"
                ...
                className="w-32 accent-emerald-600 h-1 bg-zinc-200 rounded-lg appearance-none cursor-pointer dark:bg-zinc-700"
              />
            </div>
            ...
            {project.status !== 'COMPLETED' && (
              <Button
                ...
                disabled={updateProjectMutation.isPending}
                className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold h-9 px-3"
              >
                Mark Completed
              </Button>
            )}
        ...
        {/* Description */}
        <div className="mt-6 relative z-10 pt-5 border-t border-border">
          <h3 className="text-xs font-bold text-zinc-800 dark:text-zinc-200 mb-2">Project Overview</h3>
        ...
        <div className="mt-6 relative z-10 space-y-1.5">
          <div className="flex justify-between items-center text-[10px] font-bold text-zinc-450 uppercase tracking-wide">
            <span>Milestone Progress</span>
            <span>{project.progress}%</span>
          </div>
          <div className="w-full bg-zinc-100 dark:bg-zinc-800/80 rounded-full h-2 overflow-hidden border border-zinc-200/40 dark:border-zinc-850">
            <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left column - credentials if Website Development */}
            ...
            <div className="premium-card border border-border bg-card rounded-2xl p-6 shadow-card">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-450 dark:text-zinc-500 mt-0.5">
                      Website credentials for development access
                    </p>
                  </div>
                </div>
                {project.credentials && (
                  <Button
                    size="sm"
                    variant="outline"
                    className="border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-900 text-xs font-semibold gap-1.5 h-8 px-3"
              ...
              {project.credentials ? (
                <div className="space-y-4">
                  {/* Dev URL */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-150 dark:border-zinc-850 rounded-xl">
                    <div className="min-w-0 flex-1">
                      <span className="text-[9px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider block mb-0.5">
                        Development URL
                      </span>
                      <a
                        href={project.credentials.devUrl.startsWith('http') ? project.credentials.devUrl : `https://${project.credentials.devUrl}`}
                        target="_blank"
                        rel="noreferrer"
                        className="text-xs text-emerald-600 dark:text-emerald-400 hover:underline inline-flex items-center gap-1 truncate max-w-full font-medium"
                      >
                    ...
                    <Button
                      size="icon"
                      variant="ghost"
                      onClick={() => copyToClipboard(project.credentials!.devUrl, 'URL')}
                      className="w-8 h-8 rounded-lg border border-zinc-200 hover:bg-zinc-100 self-end sm:self-auto shrink-0 dark:border-zinc-800"
                    >
                      {copiedField === 'URL' ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {/* Username */}
                    <div className="flex flex-col justify-between p-3 bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-150 dark:border-zinc-850 rounded-xl">
                      <div>
                      ...
                      <div className="flex justify-end mt-2 pt-2 border-t border-zinc-200/50 dark:border-zinc-800/50">
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => copyToClipboard(project.credentials!.username, 'Username')}
                          className="text-[10px] font-bold text-zinc-500 hover:text-zinc-700 h-6 px-2 gap-1"
                        >
                    ...
                    <div className="flex flex-col justify-between p-3 bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-150 dark:border-zinc-850 rounded-xl">
                      <div>
                        ...
                        <span className="text-xs font-mono font-bold text-zinc-800 dark:text-zinc-200 block truncate">
                          {showPassword ? project.credentials.password : '••••••••••••'}
                        </span>
                      </div>
                      <div className="flex items-center justify-end gap-1.5 mt-2 pt-2 border-t border-zinc-200/50 dark:border-zinc-800/50">
                        <Button
                          size="sm"
                          variant="ghost"
                  ...
                  {project.credentials.notes && (
                    <div className="p-3 bg-zinc-55 bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-150 dark:border-zinc-850 rounded-xl">
                      <span className="text-[9px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider block mb-1">
                        Access Notes
                      </span>
                ...
                <div className="flex flex-col items-center justify-center py-10 px-4 text-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl">
                  ...
                  <Button
                    size="sm"
                    className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold gap-1.5 h-8 px-4"
                    onClick={handleOpenCredentialsModal}
                  >
            ...
            <div className="premium-card border border-border bg-card rounded-2xl p-6 shadow-card flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-zinc-500 dark:text-zinc-400 leading-relaxed mt-0.5">
                  This project is designated as a <span className="font-semibold">{project.type.replace('_', ' ')}</span> project. Development credential drawers are specifically reserved for website development projects.
                </p>
              </div>
            </div>
          )}
      ...
      <div className="mt-6">
        <ProjectChat projectId={id} accent="emerald" />
      </div>
          ...
          <form onSubmit={handleSaveCredentials} className="space-y-4 my-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className="h-9 text-sm focus-visible:ring-emerald-500 font-mono"
                  required
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                rows={3}
                className="text-sm focus-visible:ring-emerald-500 resize-none"
              />
            </div>
